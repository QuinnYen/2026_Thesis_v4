...
3. [Method 2: IARN](#3-method-2-iarn-inter-aspect-relation-network)
4. [Method 3: HSA](#4-method-3-hsa-hierarchical-syntax-attention)
5. [模型比較總覽](#5-模型比較總覽)
6. [統一訓練配置](#6-統一訓練配置)
...
## 3. Method 2: IARN (Inter-Aspect Relation Network)
...
| 特性 | BERT-CLS | Hierarchical | IARN | HSA |
|------|----------|--------------|------|-----|
| **核心思想** | 直接使用 [CLS] | 融合 BERT 多層特徵 | 建模 aspect 間關係 | 語法驅動的層級注意力 |
| **特徵來源** | 最後一層 | 12 層分組融合 | 12 層 + aspect 交互 | BERT + 語法聚合 |
| **Aspect 處理** | 獨立 | 獨立 | 交互式 | 獨立 + 語法感知 |
| **額外結構** | 無 | 3 個融合網路 | 注意力 + Gating | 語法層 + 融合層 |
...
| Hierarchical | 通用場景 | 多粒度特徵 | 無 aspect 交互 |
